...
def init_config():
    ...
    config.barbican_sqlalchemy_url = CONF.sql_connection
    return config


def upgrade(to_version='head'):
    alembic_cfg = init_config()
    command.upgrade(alembic_cfg, to_version)


def downgrade(to_version):
    alembic_cfg = init_config()
    command.downgrade(alembic_cfg, to_version)


def generate(autogenerate=True, message='generate changes'):
    alembic_cfg = init_config()
...
------
def process_cli_cmmd(config, cmd):
    print 'cmd:',cmd
    args = cmd.split(' ')
    do_alembic_command(config, 'branches')
...
def add_command_parsers(subparsers):
    ...
    parser.set_defaults(func=process_cli_cmmd)
